...
class Sanitize
{
    ...
    public function __construct($data = [], $filter = FILTER_DEFAULT, array $options = [], Validate $validate)
    {
        ...
        $this->data = empty($this->data) ? [] : $this->scrub($this->data, $filter, $options);
    ...
    private function readFiltersAndFlags(array $options, $filter = DEFAULT_FILTER, $flags = [])
    {
        ...
        $parameters = array_merge_recursive($options, ["flags" => $_flags]);

        return [$filter, $_flags, $parameters];
    ...
    /**
     * Returns the first word a sanitised string
